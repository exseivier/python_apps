...
then
	manager split-genome -i $sequences -s $size -S $step -o split_out/${split_seqs_file}
else
...
##		MANAGER :: HIDE_MATCHED_SEQS BEGINS.
	...
	#### Do not ask me why, please.
		...
		echo -e "\n[MANAGER::HIDE_MATCHED_SEQS][BEGINS]\n"
		manager select-sequences -i split_out/input_seqs.${PID}.txt -hd blast_out/$(basename ${sequences%.*}).blast -o split_out/tmp.${PID}.txt
